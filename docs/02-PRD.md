...
Al marcarla, el servidor toma el porcentaje del mes contractual y calcula el descuento por unidad sobre el precio neto (después del descuento del asesor) de cada producto cuya categoría sea `Híbrido` o `Semilla`. Nucle no aplica a Agroquímicos ni otras categorías y se acumula independientemente después del descuento del asesor. El total nunca puede ser negativo.
...
Criterio de aceptación: con Híbrido mensual `900`, descuento asesor `100` y Nucle `10%`, el precio final unitario es `720` (ya que se calcula 10% sobre 800); un Agroquímico de `500` incluido en la misma cotización permanece en `500`.
